...
describe("AmazonClient", () => {
  ...
  describe("checkAuthStatus", () => {
    ...
    it("persists tracked Set-Cookie headers returned by auth checks", async () => {
      ...
      // Amazon rotates cookies mid-session regardless of how they were
      // obtained, so ingesting Set-Cookie stays part of the transport.
      expect(client["headers"]["x-amzn-sessionid"]).toBe("200-auth-check");
      expect(client["cookies"]["session-token"]).toBe(
        "rotated-from-auth-check",
      );
    });
  });
