...
- **Admin server has no authentication**, by design — the security boundary is the network. What it does have are three guards in [server/src/server/security.ts](server/src/server/security.ts), enforced in the router before any dispatch, which close the *remote* vector (a page the admin merely visits driving the API from their browser): **(1)** no `Access-Control-*` headers anywhere, so cross-origin reads are blocked and `OPTIONS` returns 405 — the UI is same-origin in prod and via the Vite dev proxy, so it needs none; **(2)** every non-GET/HEAD request must carry `X-Requested-With: alexa-photos`, which is not CORS-safelisted, so a cross-origin caller cannot set it without a preflight that now fails; **(3)** the `Host` header must be loopback, an IP literal, or listed in `ADMIN_ALLOWED_HOSTS`, which is what stops DNS rebinding — an attacker domain pointed at this box is same-origin to the browser, so CORS cannot help, but the Host header still carries their name. A new endpoint inherits all three; the only thing to remember is that reaching the UI by hostname requires `ADMIN_ALLOWED_HOSTS`, and that the client sends the CSRF header from one place, `requestJson` in [web/src/lib/api.ts](web/src/lib/api.ts)
